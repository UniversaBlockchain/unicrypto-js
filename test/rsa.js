...
describe('RSA', function() {
  ...
  describe('key creation', function() {
    ...
    it('should unpack private key without password (unpackPlain)', async () => {
      const k1 = await PrivateKey.generate({strength: 2048});
      const packed = await k1.pack();
    ...
    it('should unpack private key with password (unpackWithPassword)', async () => {
      const k1 = await PrivateKey.generate({strength: 2048});
      const packed = await k1.pack({password: "1111", rounds: 200});

      try {
        const k2 = await PrivateKey.unpackWithPassword(packed, "1111");
      } catch(err) {
        expect.fail();
      }
    });

    it('should unpack 8192 key', async () => {
      const packed = "";
      const key = await PrivateKey.unpack(decode64(packed));

      const longAddress = "sriuQrCVN3FNyH6VJEUhMD6t82duanKvBJp2ccyPGYD5gKQdofZNH7aTrQZ8aiCKuAjBRUv6";
      const shortAddress = "2dwybLfSX4vaeW4joP5odqvnpp2k1XiFcky5HB4NbEfchDYrSrZ";
